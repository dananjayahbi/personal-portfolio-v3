...
import { buildExperimentData, experimentFormSchema, type ExperimentFieldErrors } from "@/app/(admin)/experiments/validation";
import { cloudinaryClient, uploadFileToCloudinary } from "@/lib/cloudinary";
import type { ExperimentState } from "./experiment-state";

export async function createExperiment(_: ExperimentState, formData: FormData): Promise<ExperimentState> {
  ...
  const parsed = experimentFormSchema.safeParse({
  ...
  const existing = await prisma.experiment.findUnique({ where: { slug: parsed.data.slug } });
  if (existing) {
    return {
      status: 'error',
      message: 'A experiment with this slug already exists. Try a different slug or edit the existing experiment.',
  ...
  const experimentData = buildExperimentData(submission);
  ...
  return {
    status: 'success',
    message: 'Experiment published to your portfolio universe. You can refine it anytime.',
  };
}
