...
export function TechnologyManager({ technologies: initialTechnologies }: TechnologyManagerProps) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>, id?: string) => {
    ...
    const data = {
      ...
      icon: formDataToSend.get('icon') as string || "",
      ...
      {isAdding && (
        <form onSubmit={(e) => handleSubmit(e)} className="rounded-2xl border border-white/10 bg-white/5 p-6 space-y-4">
          ...
          <div className="flex gap-3">
            <button
              type="submit"
              className="inline-flex items-center gap-2 rounded-lg bg-cyan-500 px-4 py-2 text-sm text-white hover:bg-cyan-600"
            >
                  ...
                  {editingId === tech.id ? (
                    <form onSubmit={(e) => handleSubmit(e, tech.id)} className="space-y-3">
                      ...
                      <div className="flex gap-2">
                        <button
                          type="submit"
                          className="flex-1 rounded-lg bg-cyan-500 px-3 py-2 text-xs text-white"
                        >
                          Save
                        </button>
                        <button
                          type="button"
                          onClick={cancelEdit}
                          className="flex-1 rounded-lg bg-slate-700 px-3 py-2 text-xs text-white"
                        >
                          Cancel
                        </button>
                      </div>
