...
export function ProjectCard({ project }: ProjectCardProps) {
  ...
  function handleDelete() {
    ...
    startTransition(async () => {
      ...
      setError(result.message ?? "Unable to delete the project. Try again.");
    });
  }
      ...
      <div className="flex flex-1 flex-col gap-5 p-5">
        <div className="space-y-2">
          <div className="flex items-center justify-between text-xs uppercase tracking-[0.25em] text-white/40">
            ...
            <span title={`Updated ${project.updatedLabel}`}>{project.updatedRelative}</span>
          </div>
        ...
        {(project.technologies.length > 0 || project.tags.length > 0) && (
          <div className="flex flex-wrap gap-2 text-xs text-white/55">
            {[...project.technologies.slice(0, 3), ...project.tags.slice(0, 2)].map((item) => (
              <span
                key={item}
        ...
        <div className="mt-auto flex flex-wrap items-center justify-between gap-3 text-xs text-white/45">
          <span title={`Created ${project.createdLabel}`}>Created {project.createdLabel}</span>
      ...
      {confirming && (
        <div className="absolute inset-0 flex items-center justify-center bg-slate-950/90 p-6">
          <div className="flex w-full max-w-xs flex-col gap-4 rounded-2xl border border-white/10 bg-slate-900/90 p-5 text-center text-white">
            <div className="space-y-2">
              <h4 className="text-lg font-semibold">Delete project?</h4>
              <p className="text-sm text-white/60">
                This permanently removes <span className="font-semibold text-white">{project.title}</span> from your portfolio.
              </p>
            </div>
            <div className="flex flex-col gap-2 text-xs text-white/60">
              <span>Created {project.createdLabel}</span>
              <span>Last updated {project.updatedLabel}</span>
            </div>
