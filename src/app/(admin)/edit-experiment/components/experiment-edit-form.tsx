...
import { updateExperiment, type ExperimentState } from "../actions";
import { RichTextEditor } from "@/components/common/rich-text-editor";

const HERO_FOLDER = "portfolio/experiments/heroes";
const GALLERY_FOLDER = "portfolio/experiments/gallery";

const initialUpdateState: ExperimentState = { status: "idle" };
...
export function ExperimentEditForm({ experiment }: ExperimentEditFormProps) {
  const router = useRouter();
  const [state, formAction] = useActionState(updateExperiment, initialUpdateState);
  const [success, setSuccess] = useState<string | undefined>();
  const [heroAsset, setHeroAsset] = useState<HeroAsset | undefined>(() =>
    experiment.heroImage ? { preview: experiment.heroImage, name: "Current hero", existingUrl: experiment.heroImage } : undefined,
  );
  const [galleryAssets, setGalleryAssets] = useState<GalleryAsset[]>(() =>
    experiment.gallery.map((item) => ({
  ...
  const [descriptionValue, setDescriptionValue] = useState(experiment.description ?? "");
  const [isFeaturedChecked, setIsFeaturedChecked] = useState(Boolean(experiment.isFeatured));
  ...
  useEffect(() => {
    if (state.status === "success") {
      setSuccess(state.message ?? "Experiment updated successfully.");
      const timeout = setTimeout(() => {
        setSuccess(undefined);
        router.push("/edit-experiment");
      }, 1500);
      router.refresh();
  ...
  useEffect(() => {
    setHeroAsset(experiment.heroImage ? { preview: experiment.heroImage, name: "Current hero", existingUrl: experiment.heroImage } : undefined);
    setGalleryAssets(
      experiment.gallery.map((item) => ({
        id: `${item.url}-${Math.random()}`,
        preview: item.url,
    ...
    setDescriptionValue(experiment.description ?? "");
    setIsFeaturedChecked(Boolean(experiment.isFeatured));
  }, [experiment]);
  ...
  const handleSubmit = useCallback(
    (event: React.FormEvent<HTMLFormElement>) => {
      ...
      formData.set("experimentId", experiment.id);
    ...
    [descriptionValue, galleryAssets, heroAsset, isFeaturedChecked, formAction, startTransition, experiment.id, experiment.heroImage],
  );
  ...
  return (
    <form ref={formRef} onSubmit={handleSubmit} className="flex flex-col gap-8">
      ...
      <FieldGroup title="Experiment identity" description="Foundational metadata that anchors this case study across the site.">
        <TextInput label="Title" name="title" defaultValue={experiment.title} error={state.fieldErrors?.title} />
        <TextInput
          label="Summary"
          name="summary"
          rows={3}
          defaultValue={experiment.summary}
          error={state.fieldErrors?.summary}
        />
        <TextInput
          label="Slug"
          name="slug"
          hint="This maps to the experiment URL slug, e.g., /experiments/aurora"
          defaultValue={experiment.slug}
        ...
        <label className="flex items-center gap-3 text-sm text-white/70">
          ...
          />
          Highlight this experiment on the homepage hero carousel
        </label>
        <TextInput
          label="Featured order"
          name="featuredOrder"
          defaultValue={experiment.featuredOrder !== null ? `${experiment.featuredOrder}` : ""}
      ...
      <FieldGroup title="Narrative & stack" description="Craft the story and note the core technologies used.">
        ...
        <TextInput
          ...
          name="technologies"
          rows={6}
          defaultValue={experiment.technologies.join("\n")}
        ...
        <TextInput
          ...
          name="tags"
          rows={4}
          defaultValue={experiment.tags.join("\n")}
      ...
      <FieldGroup title="Links & gallery" description="Point to live experiences and supporting visuals.">
        <TextInput label="Live URL" name="liveUrl" defaultValue={experiment.liveUrl} error={state.fieldErrors?.liveUrl} type="url" />
        <TextInput
          label="Source or press URL"
          name="sourceUrl"
          defaultValue={experiment.sourceUrl}
      ...
      <FieldGroup title="SEO accents" description="Tailor how this case study surfaces in search and social previews.">
        <TextInput
          label="SEO title"
          name="seoTitle"
          defaultValue={experiment.seo.title}
        ...
        <TextInput
          ...
          name="seoDescription"
          rows={4}
          defaultValue={experiment.seo.description}
